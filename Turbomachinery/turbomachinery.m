...
% 2D plot Vtetha
v_tetha=abs(v_x.*tan(Yawangle));
f8 = figure(8);
set(f8, 'Position', [0 0 1500 400])
subplot(1,2,1)
...
title('Mass averaged Yaw angle \gamma')
...
%stator Blade entry angles
alpha2=[37.000; 39.500; 41.500; 45.000; 51.000];

%Behind stator
C3=sqrt(SPAN_DATA(:,8).^2+SPAN_DATA(:,10).^2);
alpha3=tan(SPAN_DATA(:,10)/SPAN_DATA(:,8));

%Behind rotor
C2=sqrt((SPAN_DATA(:,8).*tan(2*pi/360*alpha2)).^2+SPAN_DATA(:,8).^2);
beta2=360/(2*pi)*atan((U-SPAN_DATA(:,8).*tan(alpha2*2*pi/360))./SPAN_DATA(:,8));
v2r=((U-SPAN_DATA(:,8).*tan(alpha2*2*pi/360)).^2+SPAN_DATA(:,8).^2).^(0.5);

%in front of rotor
beta1=360/(2*pi)*atan(U./SPAN_DATA(:,8));
v1r=sqrt(SPAN_DATA(:,8).^2+U.^2);
